...
import axios from '../config/axios'; // Fix import
...
const ReportForm = () => {
  ...
  useEffect(() => {
    const checkVerification = async () => {
      try {
        const response = await axios.get('/api/auth/verify-status'); // New endpoint
        const userData = response.data;
        console.log('User verification status:', userData); // Debug
        setIsVerified(userData.isVerified);
        localStorage.setItem('user', JSON.stringify(userData)); // Update localStorage
      } catch (err) {
        console.error('Error checking verification:', err);
        setError('Failed to verify your account. Please log in again.');
        setIsVerified(false);
      } finally {
        setLoading(false);
      }
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <input
                ...
                required
                maxLength={100}
                className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus combustion-blue-500 focus:border-blue-500 transition-colors"
